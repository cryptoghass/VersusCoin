...
void Shutdown()
{
    ...
    RenameThread("VersusCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: VersusCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: VersusCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeVersusCoinamount=<n> " + _("Keep N VersusCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. VersusCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. VersusCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("VersusCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "VersusCoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of VersusCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart VersusCoin to complete") << "\n";
    ...
    nAnonymizeVersusCoinAmount = GetArg("-anonymizeversuscoinamount", 0);
    if(nAnonymizeVersusCoinAmount > 999999) nAnonymizeVersusCoinAmount = 999999;
    if(nAnonymizeVersusCoinAmount < 2) nAnonymizeVersusCoinAmount = 2;
    ...
    LogPrintf("Anonymize VersusCoin Amount %d\n", nAnonymizeVersusCoinAmount);
