...
// Copyright (c) 2009-2018 The Versus developers
...
const string strMessageMagic = "VersusCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight <= 1) {
        nSubsidy = 108900 * COIN; // premine
    }
    else if (nHeight <= 60) {
        nSubsidy = 0 * COIN; // premine confirme
    }
    else if (nHeight <= 133) {
...
static int64_t nTargetTimespan = 10 * 66;  // 10 mins
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	if(pindexLast->nHeight >= FBLOCK){
		nTargetTimespan = 2 * 66; // 2 minutes
	}
	else{
...
{
    RenameThread("VersusCoin-loadblk");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION && nBestHeight >= FBLOCK)
        {
