...
int nAnonymizeVersusCoinAmount = 1000;
...
#else
    const char* pszModule = "VersusCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VersusCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VersusCoin
    // Mac: ~/Library/Application Support/VersusCoin
    // Unix: ~/.VersusCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VersusCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "VersusCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "VersusCoind.pid"));
