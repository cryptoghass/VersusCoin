...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeVersusCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeVersusCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeVersusCoinAmount = strAnonymizeVersusCoinAmount.remove(strAnonymizeVersusCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVersusCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeVersusCoinAmount*COIN) nMaxToAnonymize = nAnonymizeVersusCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeVersusCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeVersusCoinAmount));
        strAnonymizeVersusCoinAmount = strAnonymizeVersusCoinAmount.remove(strAnonymizeVersusCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVersusCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeVersusCoinAmount == 0){
