...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVersusCoinAmount"))
        settings.setValue("nAnonymizeVersusCoinAmount", 1000);
    nAnonymizeVersusCoinAmount = settings.value("nAnonymizeVersusCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeVersusCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeVersusCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeVersusCoinAmount:
            return QVariant(nAnonymizeVersusCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeVersusCoinAmount:
            nAnonymizeVersusCoinAmount = value.toInt();
            settings.setValue("nAnonymizeVersusCoinAmount", nAnonymizeVersusCoinAmount);
            emit AnonymizeVersusCoinAmountChanged(nAnonymizeVersusCoinAmount);
            break;
        default:
            break;
        }
