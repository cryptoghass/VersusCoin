...
RUN source /opt/qt*/bin/qt*-env.sh && qmake CONFIG+=release Versuscoin.pro
RUN make -j$(nproc)
RUN mkdir app_image/
RUN cp versuscoin-qt app_image/
...
RUN source /opt/qt*/bin/qt*-env.sh && VERSION=$(git rev-parse --short HEAD) ./squashfs-root/AppRun app_image/versuscoin-qt -bundle-non-qt-libs
RUN source /opt/qt*/bin/qt*-env.sh && VERSION=$(git rev-parse --short HEAD) ./squashfs-root/AppRun app_image/versuscoin-qt -appimage

RUN curl -sS --upload-file ./Application*.AppImage https://transfer.sh/versuscoin-Qt.AppImage
